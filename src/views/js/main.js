...
var resizePizzas = function(size) {
  ...
  changeSliderLabel(size);
    ...
    var ChangedPizzas = document.getElementsByClassName("randomPizzaContainer");
    var ChangedPizzasLength = ChangedPizzas.length;

    for (var i = 0; i < ChangedPizzasLength; i++) {
...
var pizzasDiv = document.getElementById("randomPizzas"); //moved pizzasDiv outside of for loop, as it doesn't rely on the loop.
...
// Added phasearray so i%5 wouldn't need to be calculated over and over again.
// Moves the sliding background pizzas based on scroll position
function updatePositions() {
  frame++;
  var scrollTop = document.body.scrollTop;
  var phasearray = [];
  var phase; // declared phase outside of for loop so it isn't created every time the loop is executed.
  var length = items.length; // saved items.length in local variable to prevent continuous reacccessing in for loop
